...
A decision tree is a hierarchical model for supervised learning whereby the local region is identified in a sequence of recursive splits in a smaller number of steps. A decision tree is composed of internal decision nodes and terminal leaves. Each decision node implements a test function with discrete outcomes labeling the branches. Given an imped, at each node, a test is applied and one of the branches is taken depending on the outcome
